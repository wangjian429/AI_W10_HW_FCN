...
flags = tf.app.flags
flags.DEFINE_string('data_dir', '/home/ai/work/W10/VOCdevkit/VOC2012', 'Root directory to raw pet dataset.')
...
def dict_to_tf_example(data, label):
    ...
    feature_dict = {
        ...
        'image/filename': tf.train.Feature(bytes_list=tf.train.BytesList(value=[data[-15:-4].encode('utf8')])),
...
def create_tf_record(output_filename, file_pars):
	...
	# python 3.6 zip格式需要用list转换
	file_pars_list = list(file_pars)
	for idx, example in enumerate(file_pars_list):
		if idx % 100 == 0:
			logging.info('On image %d of %d', idx, len(file_pars_list))
		try:
			tf_example = dict_to_tf_example(file_pars_list[idx][0], file_pars_list[idx][1])
			writer.write(tf_example.SerializeToString())
		except AttributeError:
			logging.warning('Invalid example: %s, ignoring.', file_pars_list[idx][0])
